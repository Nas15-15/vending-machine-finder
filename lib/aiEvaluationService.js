...
export async function evaluateLocationsWithAI(locations) {
  ...
  if (getGeminiKey() && getGemini()) {
    try {
      ...
      console.log(`[AI Evaluation] Gemini Status: ${response?.response?.promptFeedback || 'OK'}`);
      content = response.response.text();
      console.log(`[AI Evaluation] Gemini returned ${content ? content.length : 0} characters`);
    } catch (error) {
      console.error(`[AI Evaluation] Gemini failed: ${error.stack || error.message}`);
